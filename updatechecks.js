...
M.mod_learningtimecheck = {
    ...
    init: function(Y, url, sesskey, cmid, updateprogress) {
        ...
        this.updatelist = [];
        var items = YD.getElementsByClassName('learningtimecheckitem');
        for (var i=0; i<items.length; i++) {
            YE.addListener(items[i], 'click', function (e) {
                M.mod_learningtimecheck.check_click(this, e);
            });
        ...
        window.onunload =  function(e) {
            M.mod_learningtimecheck.send_update_batch(true);
        };
    },
    ...
    update_progress_bar: function() {
        ...
        var prall = YD.get('learningtimecheckprogressall');
        var prreq = YD.get('learningtimecheckprogressrequired');

        var allpercent = (this.optionalchecked + this.requiredchecked) * 100.0 / (this.optionalcount + this.requiredcount);
        var inner = YD.getElementsByClassName('learningtimecheck_progress_inner', 'div', prall)[0];
        var inneranim = YD.getElementsByClassName('learningtimecheck_progress_anim', 'div', prall)[0];
        ...
        var disppercent = YD.getElementsByClassName('learningtimecheck_progress_percent', 'span', prall)[0];
    ...
    update_server: function(itemid, state) {
        ...
        this.updatetimeout = setTimeout(function() {
            M.mod_learningtimecheck.send_update_batch(false);
        }, 500);
        this.show_spinner();
    },
    ...
    show_spinner: function() {
        this.Y.one('#learningtimecheckspinner').setStyle('display', 'block');
    },

    hide_spinner: function() {
        this.Y.one('#learningtimecheckspinner').setStyle('display', 'none');
    }
}
//});
