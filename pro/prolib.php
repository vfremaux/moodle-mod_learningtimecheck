...
define('MOD_LEARNINGTIMECHECK_COMPONENT_PROVIDER_ROUTER_URL', 'http://www.mylearningfactory.com/providers/router.php');

require_once($CFG->dirroot.'/mod/learningtimecheck/lib.php');

final class pro_manager {

    private static $component = 'mod_learningtimecheck';
    private static $componentpath = 'mod/learningtimecheck';
    ...
    public static function add_settings(&$admin, &$settings) {
        ...
        $PAGE->requires->js_call_amd('mod_learningtimecheck/pro', 'init');
    ...
    public static function send_empty_license_signal() {
        ...
        if ((mod_learningtimecheck_supports_feature() == 'pro') && empty($config->licensekey)) {
            if ($config->licensekeycheckdate < time() - 30 * DAYSECS) {

                $url = MOD_LEARNINGTIMECHECK_COMPONENT_PROVIDER_ROUTER_URL;
